...
// Call CGQC_BasicDisplay.Show(...) from client side.
// ============================================================

class CGQC_BasicDisplay
{
	protected static const ResourceName LAYOUT = "{CF3057165B961942}UI/layouts/CGQC_BasicDisplay.layout";
	...
	static void Show(string title, string body, float duration = SHOW_DURATION)
	{
		...
		GetGame().GetCallqueue().Remove(CGQC_BasicDisplay.StartFade);
		GetGame().GetCallqueue().CallLater(CGQC_BasicDisplay.StartFade, (int)(duration * 1000), false);
	}
	...
	static void StartFade()
	{
		...
		GetGame().GetCallqueue().CallLater(CGQC_BasicDisplay.Hide, (int)(FADE_DURATION * 1000) + 50, false);
	}
